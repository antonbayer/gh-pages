!function(){
	...
	var c1=[-60, 38], c2=[-40, 10], c3=[-10, 88]; //Column positions of labels.
	var line = false;
	var colors =["#4EA13E", "#93BA39",  "#F0C429","#E67D2E", "#E63932"];
	opacity = {down: 0.3, up: 0.7};
	var svg;
	...
	function drawPart(data, id, p){
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.style("fill",function(d){
				return colors[ c1[p] < 0 ? d.key1 : d.key2 ];
			});
	}
	
	function drawEdges(data, id){
		var elm = d3.select("#"+id);
		elm.append("g").attr("class","edges").attr("transform","translate("+ b+",0)");

		elm.select(".edges").selectAll(".edge")
			.data(data.edges).enter().append("polygon").attr("class","edge").attr("points", edgePolygon)
			.each(function(d) {
			
			
					var gid = "gradient_"+d.key1+"_"+d.key2;
					...
					gradient.append("svg:stop")
						.attr("offset", "0%")
						.attr("stop-color", colors[d.key1])
						.attr("stop-opacity", 1);
					gradient.append("svg:stop")
						.attr("offset", "50%")
						.attr("stop-color", colors[d.key2])
						.attr("stop-opacity", 1);
						
				d3.select(this).style("fill","url(#"+gid+")");
				
				var op = d.key1 < d.key2 ? opacity.down : opacity.up;
	...
	function drawHeader(header, id){
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text(" ").attr("x", (c2[d]-10))
				.attr("y", -5).style("fill","grey");
	...
	function transitionEdges(data, id){
		...
		d3.select("#"+id).select(".edges").selectAll(".edge").data(data.edges)
			...
			var op = d.key2 > d.key1 ? opacity.down : opacity.up;
			
			return (d.h1 ==0 || d.h2 == 0 ? 0 : op);});	
	}
